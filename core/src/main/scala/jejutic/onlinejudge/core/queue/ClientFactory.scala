package jejutic.onlinejudge.core.queue
...
import jejutic.onlinejudge.core.config.{RabbitClientConfig, RabbitQueueConfig}
import jejutic.onlinejudge.core.domain.error.MessageCantBeRoutedException
import org.typelevel.log4cats.Logger
import org.typelevel.log4cats.syntax.*

import java.nio.charset.StandardCharsets

// +- the same file as `jejutic.onlinejudge.gateway.queue.ClientFactory`
    ...
    for {
      ...
      publisher <- Resource.eval(
        client.createPublisherWithListener[AmqpMessage[String]](
          cfg.resultQueue.exchangeName,
          cfg.resultQueue.routingKey,
      ...
      ackerConsumer <- Resource.eval(
        client.createAckerConsumer(cfg.answerQueue.name)(using
          channel,
